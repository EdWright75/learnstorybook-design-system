...
<Meta
  ...
  argTypes={{
    ...
    username: {
      description:
        "Avatar falls back to the user's initial when no image is provided.  Supplu a `username` and omit `src` to see what this looks like.",
      table: {
        defaultValue: {
          summary: "loading",
        },
...
## Displays an image that represents a user or organisation
...
Avatar is used to represent a person or organisation. By default the avatar shows an image and gracefully falls back to the first initial of the usernam. While hydrating the component you may find it useful to render a skeleton template to indicate that Avatar is awaiting data. Avatars can be grouped with the AvatarList component.
